...
local SapphireLib = {
	...
	Red = {
		["Color Hub 1"] = ColorSequence.new({
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(90, 0, 0)),
			ColorSequenceKeypoint.new(0.50, Color3.fromRGB(255, 0, 0)),
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(90, 0, 0))
		}),
		["Color Hub 2"] = Color3.fromRGB(140, 0, 0),
		["Color Stroke"] = Color3.fromRGB(0, 60, 255),
...
local Settings = SapphireLib.Settings
local Flags = SapphireLib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(SapphireLib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then SapphireLib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then SapphireLib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then SapphireLib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local Connections, Connection = {}, SapphireLib.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(SapphireLib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = SapphireLib.Themes[SapphireLib.Save.Theme]

local function AddEle(Name, Func)
	SapphireLib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = SapphireLib.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function SapphireLib:GetIcon(index)
...
function SapphireLib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	SapphireLib.Save.Theme = NewTheme
	SaveJson("redz library V5.json", SapphireLib.Save)
	Theme = SapphireLib.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(SapphireLib.Instances, function(_,Val)
...
function SapphireLib:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function SapphireLib:MakeWindow(Configs)
	...
	local UISizeX, UISizeY = unpack(SapphireLib.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, SapphireLib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			SapphireLib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("redz library V5.json", SapphireLib.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		SapphireLib.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("redz library V5.json", SapphireLib.Save)
	end)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			SapphireLib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(SapphireLib.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		TIcon = SapphireLib:GetIcon(TIcon)
		...
		local function Tabs()
			...
			table.foreach(SapphireLib.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(SapphireLib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(SapphireLib.Options, {type = "Section", Name = SectionName, func = Section})
